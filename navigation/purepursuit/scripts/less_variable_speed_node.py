...
class VariableSpeedNode(object):
    '''
    This node is meant for calculating a 
    variable speed in order to show the 
    ...
    def __init__(self):
	'''
	Initializes by setting publisher and 
        set the start time which will be used
	...
	'''
	Used the difference in time start and now
        to change the speed. The speed is centered 
        at 3.35 which is the midpoint between the 
        lookaway velocity bounds.
	...
	speed = 3.5 + .05 * math.cos(thetaTime)
        #speed now doesn't vary much at all
	speed_msg = Float64()
...
if __name__ == "__main__":
    ...
    # need to set log_level=rospy.DEBUG above for messages
